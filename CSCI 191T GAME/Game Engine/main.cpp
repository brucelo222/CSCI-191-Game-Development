...
//	scene->resizeScreen(LOWORD(lParam),HIWORD(lParam));			// Set Up Our Perspective GL Screen

	if (!game->Initialize())							// Initialize Our Newly Created GL Window
	{
		KillGLWindow();								// Reset The Display
...
{
    game->winInputs(hWnd,uMsg,wParam,lParam);
	switch (uMsg)									// Check For Windows Messages
	{
		...
		case WM_SIZE:								// Resize The OpenGL Window
		{
			...
			game->resizeScreen(LOWORD(lParam),HIWORD(lParam));
			return 0;								// Jump Back
		}
	}
					...
					int			nCmdShow)			// Window Show State
{
	...
	{
	    // REQUIRED IN WINMAIN, CHECKS IF A QUIT WAS ISSUED OTHERWISE DISPATCHES A MSG.
		...
		//call to scene manager loops through gameloop physics,render,camera,etc
